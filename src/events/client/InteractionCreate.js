	...
	run: async (interaction, DiscordClient) => {
		...
		if (interaction.isChatInputCommand()) {
			...
			if (selectMenuCollection.get(interaction.values[0])) {
				...
				if (Command.expireAfter && !isNaN(Command.expireAfter)) {
					const expireAfterCollection = DiscordClient.expireAfter;
					if (!expireAfterCollection.get(`${Command.name}_SelectMenu_Value`))
						expireAfterCollection.set(`${Command.name}_SelectMenu_Value`, Date.now());
				}
				...
				if (Command.expireAfter && !isNaN(Command.expireAfter)) {
					const expireAfterCollection = DiscordClient.expireAfter;
					if (!expireAfterCollection.get(`${Command.name}__SelectMenu`))
						expireAfterCollection.set(`${Command.name}__SelectMenu`, Date.now());
				}
			...
			if (Command.expireAfter && !isNaN(Command.expireAfter)) {
				const expireAfterCollection = DiscordClient.expireAfter;
				if (!expireAfterCollection.get(`${Command.name}_ButtonCommands`))
					expireAfterCollection.set(`${Command.name}_ButtonCommands`, Date.now());
			}
