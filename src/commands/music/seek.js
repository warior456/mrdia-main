...
async function seek (message, client, Discord, args, cmd, guildQueue) {
    try {
        ...
        if (args[0] * 1000 < guildQueue.nowPlaying.milliseconds) {
            ...
            Reply.send(`Set the time to ${args[0]} seconds`)
        } else {
            Reply.send(`you can't seek higher than ${guildQueue.nowPlaying.milliseconds / 1000} seconds with this song!`)
        }

    } catch (error) {
        Reply.send(`Something went wrong when seeking!`);
        console.log(error);
    }
}